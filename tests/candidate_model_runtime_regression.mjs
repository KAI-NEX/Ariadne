...
assert.match(pages, /saved-candidate-source-select/);
assert.match(pages, /RawSource\.resolveArchive\(database, sourceId, "CANDIDATE"\)/);
...
assert.match(html, /<select id="saved-candidate-source-select"/);
...
assert.match(html, /已保存在本机的原件/);
assert.match(html, /选择已保存的原件/);
assert.match(pages, /选择已保存的原件/);
...
assert.doesNotMatch(styles, /data-candidate-import-runtime="model-unavailable"[^}]*pointer-events: none/);
...
assert.match(html, /id="candidate-working-edit-title"[^>]*aria-required="true"/);
assert.match(html, /id="candidate-card-title-required-dialog"/);
assert.match(html, /标题必须填写。/);
assert.match(html, /id="candidate-card-title-required-confirm" class="v1-dialog-dismiss"[^>]*>确认<\/button>/);
...
assert.match(pages, /candidateExecutionState = activeCandidateWorkingModel \? "COMPLETE" : "ARCHIVED"/);
assert.match(pages, /function requestCandidateCardBack\(\)/);
assert.match(pages, /candidate-card-unsaved-dialog/);
assert.match(pages, /function requireCandidateWorkspaceTitle\(\)[\s\S]*titleInput\.value\.trim\(\)[\s\S]*candidate-card-title-required-dialog[\s\S]*return false/);
assert.match(pages, /function persistCandidateWorkspaceEdit\(\)[\s\S]*if \(!requireCandidateWorkspaceTitle\(\)\) return null;[\s\S]*CandidateModel\.editedCandidateWorkingModel/);
assert.match(pages, /function requestCandidateCardBack\(\)[\s\S]*if \(!requireCandidateWorkspaceTitle\(\)\) return undefined;[\s\S]*candidate-card-unsaved-dialog/);
assert.match(pages, /function requestCandidateWorkspaceExit\(destination\)[\s\S]*if \(!requireCandidateWorkspaceTitle\(\)\) return undefined;[\s\S]*candidate-workspace-close-dialog/);
assert.match(pages, /candidate-card-title-required-confirm[\s\S]*candidate-card-title-required-dialog[\s\S]*candidate-working-edit-title[\s\S]*focus\(\)/);
