...
(async()=>{
  ...
  try {
    ...
    await page.goto(base+'/jd.html');await page.waitForSelector('#job-card-grid');
    await page.evaluate(async()=>{const D=JobRadarV1Demo;for(const [id,title] of [['qa-chip-ai-pm','AI 产品经理'],['qa-chip-design','体验设计师']])await D.put(D.DEMO_STORES.jobs,{...D.clone(D.JOB_FIXTURE),job_context_id:id,title,company:'合成测试公司',copy_locale:'zh-CN',imported_from:{source_url:id==='qa-chip-ai-pm'?'https://jobs.example.com/openings/ai-product-manager?from=ariadne':null}});});
    ...
    assert.equal(await page.locator('#job-stage-filters').count(),0);
    const sourceLink=page.getByLabel('打开AI 产品经理的原始职位链接');
    assert.equal(await sourceLink.getAttribute('href'),'https://jobs.example.com/openings/ai-product-manager?from=ariadne');
    ...
    const result={checks:['animated four-option chip','arrow rotation','keyboard End/Escape','reduced motion','filters removed','source link visible and independent','one-step save','no detail opened by stage','closed-last order and reload','embedded notes save/cancel','source card return','reopen preserves notes/history','storage abort rollback','cross-page dirty conflict and retry','standalone detail reload','source records unchanged','desktop/mobile alignment'],alignment,record:await record(),errors,posts};
