...
for (const script of [demoDomain, localFirst, localJobs, careerEvidence, careerProfile]) assert.match(script, /DB_VERSION = 12/);
for (const store of ["runtime_snapshots", "extraction_artifacts", "processing_batches", "context_proposals", "context_review_decisions", "candidate_context_revisions", "candidate_context_lifecycle", "job_context_revisions", "candidate_contexts", "candidate_proposals", "candidate_context_patches", "processing_runs", "processing_consents", "conversation_sessions", "conversation_messages", "demo_candidate_items", "demo_job_contexts", "demo_conversations", "demo_ui_state"]) {
...
console.log(JSON.stringify({
  ...
  indexeddb_version_path: "unified_v12_additive_truth_stores_pass",
}, null, 2));
