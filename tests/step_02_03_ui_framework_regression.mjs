...
assert.match(pages, /panel\.scrollIntoView\(\{ behavior: "smooth", block: "center" \}\)/);
...
assert.match(pages, /const batchId = `batch-candidate-extraction-\$\{crypto\.randomUUID\(\)\}`/);
...
assert.match(pages, /Promise\.allSettled\(Array\.from\(files \|\| \[\]\)\.map/);
...
assert.match(pages, /await processJobSource\(source, batchAuthority, jobBatchAbortController\.signal\)/);
assert.match(pages, /async function processJobSource\(source, batchAuthority, signal\)[\s\S]*LocalJobLifecycle\.persistPendingImport/);
...
assert.match(pages, /LocalContextLifecycle\.prepareTextSource/);
