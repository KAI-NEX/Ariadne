...
assert.match(pages, /byId\("job-file-input"\)\.addEventListener\("change", \(event\) => \{[\s\S]*acceptJobFiles\(event\.target\.files\)/);
...
assert.match(pages, /const batchKey = `job-source-\$\{crypto\.randomUUID\(\)\}`/);
assert.match(pages, /source_key: `\$\{batchKey\}-\$\{index \+ 1\}`/);
...
assert.match(pages, /\["pdf", "png", "jpg", "jpeg", "docx"\]\.includes\(extension\)/);
assert.match(pages, /installFileDropzone\("job-dropzone", acceptJobFiles\)/);
...
assert.equal((pages.match(/completeEmbeddedImport\("jd"/g) || []).length, 1);
assert.doesNotMatch(jobImport, /支持多文件|可同时上传多个|批量上传/);
assert.match(pages, /source_type: "PASTED_TEXT_METADATA"/);
