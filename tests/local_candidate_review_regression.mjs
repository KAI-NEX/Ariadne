...
assert.match(pages, /candidateExecutionState === "ARCHIVED"/);
assert.match(pages, /已恢复原件；尚未发起新的分析。/);
...
assert.match(pages, /setCompletedSourceSheet\(false\)/);
...
assert.match(pages, /button\.disabled = candidateProcessingInProgress \|\| !selectedCandidateSources\.length \|\| \(!modelReady && candidateExecutionState === "ARCHIVED"\)/);
