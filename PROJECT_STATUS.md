...
## 2026-09-21 — 对话附件发送后仍留在输入框（COMPLETE；本机与 GitHub 同步）
...
- 119/119 离线回归、VI、专项 transport/附件测试和真实浏览器合成附件 dispatch/失败恢复通过；浏览器 QA 没有模型请求或页面异常。本机 Skill 与 GitHub 发布包在本阶段收口时同步；未把用户截图或私人附件用于测试，未部署官网或 API。
