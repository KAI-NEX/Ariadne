...
## 2026-09-14 — 官网 8.5 秒循环与 3 秒叠映（COMPLETE；本地验证，未发布）

- 用户先要求一圈 8.5 秒，随后将淡入淡出最终指定为约 3 秒。本版保留原慢动作中间片与画风，将 smoothstep 叠映扩为 72 帧，生成 `ariadne-original-soft-loop-v2.mp4`（205 帧 / 24 fps / 8.541667 秒，约 4.07 MB）；页面按媒体 duration / 8.5 设置播放速率，因此实际一圈 8.5 秒，未叠映游动约 5.51 秒、叠映约 2.99 秒。
- 速度在元数据就绪和重新可见时设定，保留单视频原生循环，不逐圈重载或重置进度。复用已验证的无损慢动作，未重新估计运动；鱼与光影使用同一整帧叠映。约 1.1 秒过渡试片及所有旧素材原地保留，未作为最终页面资源。
- 媒体回归通过：93 个原始时间锚点误差 0，普通游动无定格，整段鱼群区域最大相邻变化 2.41，循环接缝 1.06；有效过渡时长与用户目标误差小于一帧。编辑与检查脚本新增可选叠映帧数，原默认参数仍支持旧版复现。
- 最终静态页面在 egolite 连续三圈：实测间隔 8.51 秒左右，0 暂停 / loadstart / 媒体错误，同一视频节点；620 帧播放统计中记录 2 丢帧。检查当前浏览器叠映画面及窄屏播放。Vite 构建、VI 静态/负向与 diff 检查通过；证据位于 `.cache/website-soft-loop-3s-20260914/`，实际时长及过渡边界见 [官网 README](website/README.md)。
- 仅提交本次速度、当前叠映视频、相关脚本和文档。其他任务修改、未采用的试片及中间素材原地保留；未 push、发布、修改域名、访问个人资料或调用模型，公网与真实手机硬件未测。

## 2026-09-14 — 官网慢速长循环（COMPLETE；本地验证，未发布）

- 用户指出 3.25 秒一圈导致叠映过于频繁，要求放慢游鱼、尽量延长间隔。当前视频改为 `ariadne-original-slow-loop-v2.mp4`：10.875 秒 / 24 fps / 261 帧，约 5.17 MB；每圈约 10.21 秒为未叠映游动，叠映仍为 16 帧 / 0.67 秒，发生频率降低约 70%。
- 原片 25–117 帧以约三分之一速度正向运动，118 帧仅用于补间前瞻；先在连续相邻原帧之间进行运动补间，再将整帧首尾 smoothstep 叠映。补间不跨内部硬切或叠映接缝；保留原鱼群、光影与页面构图，生产页面无需运行补间或逐圈切换视频。
- 媒体回归通过：93 个原始帧锚点误差为 0；无普通游动定格帧，鱼群区域普通帧变化中位数由原片 2.35 降至 1.05，整段最大变化 2.42、循环接缝 1.10。编码器前瞻造成初次输出不足预期，保留初稿，修正有效片段终点后交付 v2；不通过补静止帧凑时长。新增可复现脚本、实际帧数与时间检查。
- Vite 生产构建、VI 静态/负向及 diff 检查通过；egolite 实测当前静态资源连续六圈，间隔约 10.89 秒，1701 帧中 0 丢帧，0 暂停 / loadstart / emptied / 媒体或页面错误；同一视频节点以 playbackRate=1 播放烘焙后的慢动作。另检查窄屏与减少动态效果下的持续循环。CDP 页面截图接口两次超时，改用浏览器当前视频帧、语义快照、布局读数和逐帧图像核验本次动画，未声称取得新版完整页面截图。具体运行记录及图像保留于 `.cache/website-slow-loop-20260914/`，复现与限制见 [官网 README](website/README.md)。
- 单视频节点保持原生静音、内联、自动循环，过渡已经烘焙到视频中；原片、旧试片、无损中间片与 QA 原地保留。只提交本阶段资源、脚本及记录，不包含其他任务修改或未采用试片；未 push、部署、改域名、访问个人资料或调用模型，真实手机硬件与公网加载未测。

## 2026-09-14 — 官网叠映缩短与衔接片段优化（COMPLETE；本地验证，未发布）

- 用户撤回正反循环方案，选择叠映并要求尽量无痕；首页恢复正向运动，改用 `ariadne-original-blend-v2.mp4`。从原片内部硬切前选择 25–118 帧，尾部 103–118 与开头 25–40 帧 smoothstep 叠映，文件结尾自然衔接第 41 帧。鱼与光影一起处理，不改变几何、颜色或页面布局。
- 叠映从 24 帧缩短至 16 帧（1 秒至约 0.67 秒），使用鱼群分布更接近的衔接段；输出 78 帧 / 24 fps / 3.25 秒，约 2.68 MB。逐帧对照确认双影持续区间缩短；仍有短暂淡入淡出，不宣称每条鱼的物理轨迹闭合。
- 媒体回归通过：鱼群区域整段最大相邻变化 5.04、接缝 2.35；旧版为 4.94 / 2.30，缩短叠映未造成新的突变（对比阈值 10%）。编辑和检查脚本支持选定起点，默认参数仍兼容旧版，拒绝覆盖源及已有输出。
- 最终生产资源在 egolite 连续循环五次：同一视频节点，0 暂停、0 loadstart、0 emptied、0 页面/媒体错误；395 解码呈现统计帧中记录 2 丢帧，不宣称全平台零丢帧。切换语言不重建视频；另验窄屏、减少动态效果及连续播放。Vite 构建、VI 静态/负向及 diff 检查通过。证据位于 `.cache/website-blend-refine-20260914/`，以含 `v2` 的截图及 `browser-desktop.json` / `browser-mobile.json` 为本次交付依据。
- 原片、旧叠映、正反试片及中间 QA 原地保留；本次仅提交当前叠映资源、相关脚本和记录，其他任务改动及未采用试制继续未提交。未 push、发布、改域名、读取个人资料或调用模型；公网加载和真实手机硬件未测。

## 2026-09-14 — 原动画内部硬切与融合循环（COMPLETE；保留原画面，未发布）

- 用户在恢复原视频后要求保留当前动画、通过融合消除闪回。逐帧找到原片自身第 119 帧（约 4.958 秒）的内部硬切，之前仅处理文件末尾遗漏了这一切点；此次不再替换画风。
- 保留内部硬切前连续的 0–118 帧，用 24 帧 smoothstep 首尾柔和叠化，得到 95 帧 / 24 fps / 3.958333 秒的 `ariadne-original-blend-v1.mp4`。最后接回自然相邻源帧，持续正向游动；鱼群颜色、形状、水窗与光影均来自原片。融合段短暂叠映，不宣称独立鱼轨迹物理闭合；光流试片出现变形，未用于页面，原地保留。
- 原片内部切点鱼群区域相邻变化峰值 18.63，输出整段峰值 4.94、循环接缝 2.30，后者处于正常逐帧变化范围。新增针对这份原片的编辑与成片检查脚本，拒绝覆盖源/已有输出；原片同字节副本保留以复现。
- 最终静态构建在 egolite 连续跨过四次循环：385 帧、0 丢帧、0 暂停事件、0 页面错误。实际观察整圈及融合段，保持静音内联自动播放、隐藏暂停入口、原画面构图；Vite 构建、VI 静态/负向和 diff 检查通过。来源与短暂叠映边界见 [官网 README](website/README.md)，证据在 `.cache/website-flow-loop-20260914/`。
- 仅提交当前原动画融合与播放组件、必要布局及记录；试制源码/媒体、历史证据、无关 Mac App/域名/图谱改动原地保留。未 push、发布、访问个人资料或调用模型；真实手机硬件和公网加载未测。

## 2026-09-14 — 恢复官网最初原视频（COMPLETE；本地验证）

- 用户最新要求「回到最初的动画，只要动画能循环播放」。首页恢复 CloudFront 原始 8.041667 秒视频的本地副本 `website/public/media/ariadne-original.mp4`，SHA-256 与保留原片一致；原鱼群、颜色、光束和阴影恢复，不再引用叠化/三维素材，不承诺原片运动首尾无缝。
- 保留原生静音、内联、自动循环及前台意外暂停恢复；不提供暂停按钮，不因减少动态效果停止播放。桌面 70% / 手机 85% 的原片构图、透明导航及三页中英内容保留；移除未使用的 Three.js 依赖，试制源码、视频和 QA 原地保留。
- 最终生产构建在 egolite 实际跨过两次完整循环：loop=true、controls=false、paused=false，0 暂停事件、0 页面错误；截图确认原动画恢复。Vite 构建、VI 静态/负向及 diff 检查通过。证据在 `.cache/website-original-restore-20260914/`。
- 本次本地提交只收录恢复原视频及播放行为、必要布局和记录；未 push/部署，原有 Mac App、域名决策、图谱和三维试制未提交源码继续保留。

## 2026-09-14 — 官网连续游鱼与同步阴影试制（TRIAL；后续用户撤回，未单独提交）

- 用户明确指出上一版鱼群在叠化段闪现/消失、阴影错位；上一条媒体记录保留为历史，本项取代其动画质量结论。本次只处理动画与必要构图，不改介绍、体验或中英文内容。
- 重建 22 条鱼的闭合游动轨迹、沿速度转向与身体/尾鳍形变；一条红白主鱼，其余柔和多色。鱼与阴影来自同一帧三维几何；水窗、光束、焦散同周期，保留原片和全部试制素材。新版为三维水下插画，画风与原始摄影感视频不同。
- 为避免实时 WebGL 受浏览器后台节流影响，逐帧输出 1080×1200 / 30 fps / 720 帧 / 24 秒 MP4；不叠化、不倒放、不重复终点帧。Three.js 仅作开发渲染依赖，生产首页 JS 不携带它。编码 v1 接缝细节变化偏大，保留原地；v2 固定 I/P/B 量化关系，当前选定文件约 27.2 MB。
- 首页移除暂停按钮、原生控制和减少动态效果自动暂停；前台意外暂停或重新可见时恢复播放，不重置播放位置。手机完整场景放在标题与底部说明之间，导航不遮水窗、底部文字不叠在亮色地面上。普通页面淡入淡出与减少动态效果边界保持。
- 轨迹回归检查两圈全部鱼的位移、朝向、闭合和尾鳍相位；源第 0 / 720 帧最大通道差 1/255。成片 720 帧全部发生变化，接缝像素平均变化 1.047，低于正常帧峰值 1.139；接缝与前一帧变化比 1.330，通过预先设定的 1.4 上限。
- egolite 最终静态构建完整跨过两次循环：1,448 帧、0 丢帧、0 暂停事件、0 页面错误。桌面 1440×900、手机 390×844 / 320×640 无横向溢出；验证减少动态效果仍播放、意外暂停恢复、菜单 → 介绍 → 首页重新播放。Vite 构建、VI/负向/CSS token/diff 检查通过。
- 来源、复现与限制见 [官网 README](website/README.md)，源帧、旧素材与 QA 在 `.cache/website-v3-20260914/`；浏览器后台/系统挂起仍受平台控制，真实手机硬件及公网加载未测。试制尚未创建 commit，后续用户要求恢复原视频；未 push、部署、修改 DNS 或个人资料，原有 Mac App、域名决策及图谱改动保留。

## 2026-09-14 — 官网三页、中英切换与循环媒体（COMPLETE；原片编辑，未发布）

- 按用户第二轮要求将官网改为「首页、介绍、体验」三个点击切换视图：取消 smooth scroll/滚动换页，沿用 Ariadne 的 320 ms 淡出和 420 ms 淡入；支持快速切换、返回、hash 深链、旧 web/download 链接、页面焦点与小屏页内阅读。
- 删除顶部探索按钮及遮罩/blur，新增紧凑 `[中/EN]`，完整翻译三页、导航、辅助标签及元数据；语言仅保存到官网自己的偏好 key。首页删除上方引导句，slogan 为 From here to anywhere / 看清自己，走向想去的地方。导航与视频分开，手机将完整水窗置于标题下方。
- 介绍聚焦分别理解、来源与未知、人工判断/保存，底部接 GitHub MIT 开源位置；体验页并列 Web 与本地源码下载。按用户即将部署的要求，Web CTA 直接使用 `https://web.ariadne.kai-nex.com`；GitHub 当前发布无独立安装包，按钮明确为「下载源码」，真实 ZIP 响应 200，不宣称存在安装程序。
- 原片本地保存后逐鱼跟踪调色，突出一条红白主鱼、其余柔和多色；157 帧 / 24 fps 的媒体用 1.5 秒首尾叠化及主鱼位置对齐缓解硬切，作为本站 hash 资源打包。保留原鱼形，未重新生成锦鲤鱼种或全新运动；衔接段存在短暂叠化，不声称物理无缝的新动画。源片、试制及 QA 原地保留；离线脚本防止覆盖原件/既有输出。
- 最终构建与 VI/负向/CSS token/diff 检查通过；egolite 桌面 1440×900、手机 390×844、320×640 无横向溢出，验证三页中英、点击/滚动边界、手机开合/Escape/焦点循环、快速跳转/返回、语言刷新、旧链接、减少动态效果与媒体失败回退。开发页跨两次视频循环仍播放；最终静态构建媒体 1920×1080/6.54 秒可解码播放，无捕获到的 JS 错误。后台帧回调有节流，不将其作为帧率保证。
- 说明与限制在 [官网 README](website/README.md)，截图/执行结果在 `.cache/website-v2-20260914/`。仅本阶段本地 commit，不 push、部署、改 DNS、访问个人资料或调用模型；原有 Mac App、域名决策、图谱等未提交内容保留。

## 2026-09-14 — Ariadne 独立官网（COMPLETE；本地验收，未发布）

- 按用户提供的 Foldcraft 参考实现 Ariadne 官网：首页、介绍、网页版、下载四个锚点区块。独立 React/Vite/Tailwind/Lucide 工程在 `website/`，不替换现有应用入口或读取工作区；首页使用用户原视频、70% 水平裁切、中文大标题与错落入场。源码、构建和发布边界见 [官网说明](website/README.md)。
- 官网正文用参考指定 Geist，中文沿用系统黑体，字标保留 Recursive；manifest 新增官网背景/字体 token，经生成器更新，内容沿用已有 VI 网格。修复实际浏览器发现的深浅区块交界导航对比度、手机视频控制可读性和锚点 4 px 偏差；未扩大历史例外。
- `pnpm build`、VI 静态和负向回归、官网 CSS 同源扫描及 diff 检查通过。egolite 验证开发与静态构建，桌面 1440×900、手机 390×844，320×640 无横向溢出；四区导航、手机开合/点击关闭/Escape/焦点循环/断点关闭、真实视频时间推进/暂停、减少动态效果与远端视频失败回退通过。点击本机入口实际到达 8000 运行选择页；未继续应用流程、改资料或调用 Provider。中英文真实字体经 CDP 核对。
- 当前 Web 域名未解析，页面标注准备中，仅 loopback 预览显示本机工作区入口；没有独立公开安装包，下载区如实说明，不把已有本机启动器作为可分发包。未发布、修改 DNS、push 或重启用户应用；公网、其他系统字体及真实模型语义不在本阶段验收范围。
- 原视频、截图、产物和 QA 结果原地保留；证据在 `.cache/website-20260914/`，构建在忽略的 `website/dist/`。仅本阶段本地 commit，已有 Mac 启动器、公开入口决策、图谱及其他未提交内容保留。
