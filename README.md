...
在 Model 模式的候选人资料工作区，可以直接说明想达到的结果，不需要记住字段名或命令格式：

- “除了声音实验，其余项目分类改成研究项目。”
- “这段经历我的角色改成研究协调，标题不变。”
- “学校的就读时间改成 2024–2026。”
- 已明确范围后可以接着说“就这样改”；只想讨论时说“先解释，不要改”。

模型结合当前资料和相关对话理解意思；代码检查目标、版本和实际执行结果。修改先进入草稿，核对卡片与修改回执后点击“保存到个人资料”，才生成确认版本。分类标签可以手动编辑或清空，清空后恢复默认类型说明。

目前对话修改限于同一来源的已有卡片字段，不支持新增、删除或合并卡片，也不跨来源修改。个人「了解我」和「了解职位概况」有各自的上下文与权限，不会因共用模型而自动获得所有修改能力。更多说明见 [对话理解与执行架构](docs/current/CONVERSATION_SEMANTIC_EXECUTION.md)。
