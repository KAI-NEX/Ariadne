...
### 2026-09-15 · README 架构截图按语言拆分

- 用户目标：中文区继续显示中文架构截图，英文区必须显示图内内容也为英文的截图和交互图。
- 实际变化：为 `02-document-understanding`、`05-content-context-simplification`、`07-architecture-evolution-six-stages` 新增独立英文 typed JSON、HTML、明暗截图和浏览器回执；英文 README 的三张预览与最终交互入口全部切换到 `.en` 版本，中文 README 保持中文版本。
- 为什么发生：README 的文字虽然已经双语，但此前两种语言引用同一份中文图，英文读者仍需跨语言阅读节点、关系、卡片和导览。
- 影响范围：公开 README、Archify 图谱索引和交付回执；不改变应用 Runtime、存储、Provider、资料或确认数据。
- 是否改图：是；拓扑和历史事实不变，只为英文排版重新收敛画布、节点宽度与说明长度。
- 验证：三图均为 Archify showcase 9/9、0 errors、0 warnings；1440×900、1600×1000、1920×1080、2048×1320 自动浏览器检查全部无溢出；明暗主题和大屏截图完成实际视觉复核。
- 修正轮次：英文 02 为 0；英文 07 为 1；英文 05 为 2。修正只处理英文长度造成的可读性和首屏高度问题。
