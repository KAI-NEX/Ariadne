...
对后续实现，本合同第 7/8/14/15 节的 Current Runtime 读取细化为「全局 Local/Model 门禁 + 当前 scope 有效选择」。显式对话覆盖不随其他对话/全局模型默认变化；同一会话、模式门禁与未覆盖默认的跨标签同步另按补充架构执行。快照不可变、整批导入冻结、领域权限、来源完整性、多模态资格和禁止静默 fallback 均不变。历史 baseline 与旧实施 slice 保留；新执行必须携带版本化 execution_settings，旧历史读取不补写强度。
