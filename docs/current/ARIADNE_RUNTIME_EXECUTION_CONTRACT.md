...
用户确认首页仅显示简洁模型名（例如「GPT Sol」），对话输入框左下提供模型/推理强度切换，后续其他模型复用同一结构。目标架构、默认/对话覆盖、参数目录、版本化快照、迁移及验收见 [统一模型选择与对话内调节](MODEL_SELECTION_AND_TUNING_ARCHITECTURE.md)。该文已记录，**控件与执行参数尚未实现**，当前 Codex 仍是固定 Sol / medium。

对后续实现，本合同第 7/8/14/15 节的 Current Runtime 读取细化为「全局 Local/Model 门禁 + 当前 scope 有效选择」。显式对话覆盖不随其他对话/全局模型默认变化；同一会话、模式门禁与未覆盖默认的跨标签同步另按补充架构执行。快照不可变、整批导入冻结、领域权限、来源完整性、多模态资格和禁止静默 fallback 均不变。历史 baseline 与旧实施 slice 保留，不据此宣称新架构已经执行。
