...
2026-09-12 标签补充：投递状态复用首页 `runtime-selector/runtime-menu/runtime-chevron` 的展开/收起、180° 箭头翻转与悬停/按下阴影；只复用动效，保留原卡片标签的 pill 圆角和 round corner-shape。宽度从固定 108 px 改为随内容（当前 84 px），文字/14 px 箭头间距 8 px，容器内 left 由 24 px 改为 22 px 作用户指定的光学对齐。支持减少动态效果，不改变持久化契约。

职位库的投递阶段（2026-09-11 修订）：卡片左上角原类型标签改为状态下拉，使用 caption、44 px 热区、胶囊圆角和既有 chevron-down 图标；选择器与正文链接是同级 DOM，视觉共用左上对齐轴，卡片悬停不位移以免脱离标签。列表只保留关注中/已结束/全部同页筛选，无卡片底部状态栏或进度弹窗。结果/备注移到详情的投递记录区域，沿用字段/文字保存取消操作与 token；浮窗内也显示保存/错误反馈，不用颜色评价投递结果。实现见 `public/job-application.css`，行为见 [职位投递阶段](JOB_APPLICATION_STAGES.md)。
...
- **整体对话开场**（2026-09-10 修订）：个人理解与职位概况内部没有固定可见标题；空对话显示「关于这些信息或许你想问」与三个胶囊入口；提示顶部对齐返回按钮热区底部，胶囊随整组上移，选择或开始第一轮发送后整体隐藏，已有历史不显示。职位第三入口转到个人对话，底部不重复导航。页面使用视口高度与独立历史滚动，输入 dock 下方保留简短的当前模型/传输范围/费用确认，距底部 8 px 或设备安全区。
- **消息线程**：Human 白底靠右、Assistant 深底靠左；最大宽度 88%；提示、输入与发送由共享组件管理。
- **发送与回复**：整体对话页未勾选底部传输说明时，发送按钮可点击并触发共用纯文字提示、焦点移至复选框；勾选后不再重复询问同一接收方，Local/无资格/忙态仍禁用。通过发送前置检查后立即清空输入，等待期间的新草稿不被返回结果清除；两个整体对话页失败时恢复未被编辑的原草稿。共用消息组件只对本轮新回复快速逐字展开（按字素约 6 ms，总时长 120–1800 ms）；历史读取不重播，重绘续接进度，来源链接和原始回复保留。减少动态效果时直接完整显示。此为已校验回复的展示动效，模型处理期间仍显示等待状态，不代表 Provider 流式传输。
- **传输勾选提示**：`ConversationUI.requireTransferConsent` 与 `.v1-consent-hint` 共用无图标、无尖角的文字 popover，替代不可控制样式的浏览器校验气泡。12 px 辅助文字、400 字重、1.65 行高，12 px 小控件圆角，上下 8 px / 左右 12 px 留白，复用 surface/line/text-secondary/card-shadow。宽度随文字、窄屏按视口收敛换行；位于勾选框上方，滚动/缩放跟随，不使用固定 z-index。勾选、离开焦点、外侧点击或 Esc 收起；保留 alert、描述关联与焦点，不改变传输权限。
- **composer**（2026-09-10 修订）：六入口共用整行输入外框，文字区默认与最小 64 px（两行 20 px 文字＋上下各 12 px 留白），底部 44 px 工具行内左侧附件/模型、右侧 42 px 发送，发送不遮挡正文。聚焦反馈由圆角 field 统一承担。上沿拖柄保留上下调整与方向键、Home/End；高度受视口与至少 80 px 历史空间限制，上限 320 px，Home 恢复两行高度。消息区独立滚动，输入 dock 悬浮可用，加载不重置用户高度。由 `ConversationUI.enhanceComposers()` 移动原有 submit，不另建发送链路。
- **对话模型选择**（2026-09-10 修订）：六入口复用 `conversation-model-selector.js/css`。透明 plus 旁为 sliders 图标与模型/强度简称；向上 popover 显示「选择模型」、首页当前 Provider 下全部已验证组合及「切换模型服务请回到首页。」短提示，选中有 check，点选即生效，不保留复杂说明或应用/默认/关闭按钮。使用既有 surface/line/dialog shadow、256 px 宽度、字号、圆角与 44 px 热区；窄屏限宽并支持滚动，方向键及 Home/End 导航，Escape 恢复焦点。首页 Provider 或 scope 变化收起陈旧菜单。Local/忙态/连接失败继续守卫，回复气泡不显示模型/强度标签，当前选择仅在输入区菜单呈现；原执行快照保留。
...
- **个人补充确认操作**（2026-09-10 修订）：复用 `v1-edit-text-action`，确认保存/确认停止使用为黑色文字、暂不采纳为灰色；正常、悬停、聚焦均无背景、边框或阴影，保留原有 44 px 热区、悬停下划线与键盘焦点。仅改变呈现，不改变人工保存或拒绝行为。
...
AI 对话等待使用用户提供的 WavePhysicsLoader 轨迹，由 `wave-physics-loader.js/css` 以原生 Web Animations 实现：15 根 12 px 柱条、8 px 间隔、201 个采样帧、4 秒线性往返，小球四次抛物线弹跳/单程与落地压缩同步驱动波峰及下压。使用 manifest 的 wave-light/dark/ball 颜色端点，支持 light/dark；2026-09-11 按用户反馈将整组视觉尺寸减半，窄屏按 .3 / .375 / .5 比例缩放。六个对话 status 共用，进度更新不重播、离屏/后台暂停，减少动态效果时静态呈现，完成/失败/离页清理动画；状态文案和辅助阅读保留。发送按钮与非对话导入加载的既有旋转动画不改。
...
### 2026-09-14 官网第二版交互与构图

按用户修改为「首页、介绍、体验」独立视图；不使用 smooth scroll 或滚动切页，复用 Ariadne 的 320 ms opacity 淡出与 420 ms 淡入。顶部移除探索按钮、背景层与 blur，加入紧凑 `[中/EN]`；导航与视频区域分开，手机将水窗移至标题下方。正文与控件保持既有 VI token，官网未改变主应用视觉资源。

介绍页说明来源、分别理解、人工判断，底部加入 GitHub；体验页用开放双列排版并列 Web 与源码下载，手机单列。中英文与 aria 标签统一放在 `website/src/content.js`。视频逐条调色、主鱼对齐与首尾叠化属于官网专用媒体，不新增通用语义颜色或图标例外；处理方式与实际限制见官网 README。


### 2026-09-14 官网游鱼动画修订

用户指出上一版叠化循环存在鱼群重影、突然消失和影子错位。本次将首页素材替换为 24 秒连续三维运动的逐帧视频：一条红白主鱼、柔和多色鱼群、水窗和光束；鱼与投影使用同一份几何/时钟，摄影感原片原地保留。三维插画材质色仅用于场景，不成为共用 UI 色值。生产页面不运行 Three.js；取消暂停入口和减少动态效果自动暂停，普通页面切换仍尊重减少动态效果。小屏场景在标题与底部说明之间完整容纳，导航继续透明。源场景、复现脚本及验收见 [官网 README](../../website/README.md)。


### 2026-09-14 官网动画最终选择：恢复原视频

用户撤回三维试制，明确要求「回到最初的动画，只要循环播放」。首页恢复原始 CloudFront 视频的本地同字节副本，保留原鱼群和影子；原生静音自动循环，不提供暂停按钮，不因减少动态效果自动暂停。恢复原视频 70% / 手机 85% 构图，导航继续透明且给水窗留空间。上述三维方案与素材仅作历史试制保留，不是当前 VI 交付。


### 2026-09-14 原画面融合循环补充

用户在恢复原视频后要求保留画面、消除闪回。逐帧确认原片第 119 帧已有内部硬切，现选取硬切前连续画面、首尾柔和叠化，输出 95 帧 / 24 fps。仅使用原鱼身与光影，融合段短暂叠映；不再尝试三维替换或会拉扯鱼身的光流变形。首页构图与播放控制维持恢复原片版本。


### 2026-09-14 叠映优化（当前选择）

用户试看正反播放后选择叠映。当前首页引用 `ariadne-original-blend-v2.mp4`：选取原片 25–118 帧，16 帧 smoothstep 叠映（约 0.67 秒），缩短双影出现时间并采用鱼群分布更接近的衔接段。原鱼群、阴影、水窗和全速正向运动保留；不拉扯几何、不改变 UI 构图。单文件原生循环，无逐圈资源切换。上述其他素材和方案仅作历史保留，具体复现及边界见官网 README。


### 2026-09-14 慢速长循环（当前选择）

用户指出上一版 3.25 秒循环导致淡入淡出频繁。本版将同一连续原片的运动放慢至约三分之一，通过相邻帧补间维持 24 fps，之后再执行固定 16 帧叠映；成片约 11 秒，约 10.21 秒为未叠映游动，过渡约 0.67 秒。补间不跨原片内部硬切或叠映接缝；原鱼群与光影一同处理。页面只使用预生成的单个 `ariadne-original-slow-loop-v2.mp4`，不新增运行时动画或改动 UI 布局。历史素材继续保留，复现见官网 README。


### 2026-09-14 循环节奏微调至 8.5 秒（当前选择）

用户要求稍微加速至每圈 8.5 秒。保留同一慢动作成片，在元数据就绪及重新可见时以 `duration / 8.5` 设定播放速度（约 1.2794 倍）。鱼群与光影一同加速，短叠映约 0.52 秒；不更换资源或重置进度，UI 构图保持。媒体本身仍为 10.875 秒，实际页面周期为 8.5 秒。


### 2026-09-14 延长 8.5 秒循环的叠映（当前选择）

用户最终要求淡入淡出延长到约 3 秒。在 8.5 秒一圈的目标下，将既有慢动作片的 smoothstep 叠映从 16 帧增至 72 帧；当前媒体为 `ariadne-original-soft-loop-v2.mp4`，页面实际过渡约 2.99 秒，普通游动约 5.51 秒。继续使用整帧同步处理鱼与光影；延长融合区间，不重做运动估计，不更改构图或页面内容。旧版资源与 QA 原地保留。
