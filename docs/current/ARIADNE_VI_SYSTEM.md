...
- **资料库编辑入口**（2026-09-20）：个人/职位库标题右侧复用 `v1-detail-edit-button` 文字「编辑 / 完成」，44 px 热区。编辑态卡片右上角显示使用 danger token 的「删除」，同一共用确认 dialog 提供取消/确认；删除按钮独立于详情链接，保留键盘焦点与窄屏换行。详见 [资料库卡片编辑与删除](CARD_LIBRARY_EDITING.md)。
