...
2026-09-25 对话呈现修订：移除下述旧版的编号技术过程折叠列表，直接展示模型公开思路说明、真实思考/网页活动状态及实时回答。新增文字复用 focus 色、透明度与 motion-open/ease-reveal；减少动效时直接显示。来源、传输和人工保存说明继续由既有入口承担，不在聊天底部重复技术免责声明。

2026-09-24 对话交互覆盖下述早期等待/勾选呈现：六入口先显示轻量的「开始对话前」说明和同意按钮，进入后不重复显示底部 checkbox 文案；只改变同意入口，不自动发送。等待改为三个 focus 色波动点，旧 WavePhysicsLoader 资源与演示保留但日常对话不再播放。实时正文直接跟随公开 delta；过程列表默认折叠，240/190 ms 展开/收起并支持中断、减少动效。收到过实时文字的最终回复不重播模拟逐字动画。共用实现 `conversation-entry-consent.*`、`conversation-dots.css`、Turn Transport；沿用本文字号、色彩与组件。
...
- **资料库编辑入口**（2026-09-20）：个人/职位库标题独立居中，页面最右侧复用 `v1-detail-edit-button` 文字「编辑 / 完成」，与最左侧返回图标使用对称的 44 px 热区。编辑态卡片右上角显示使用 danger token 的「删除」，同一共用确认 dialog 提供取消/确认；删除按钮独立于详情链接，保留键盘焦点与窄屏换行。详见 [资料库卡片编辑与删除](CARD_LIBRARY_EDITING.md)。
