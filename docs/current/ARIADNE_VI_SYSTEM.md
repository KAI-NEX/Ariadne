...
- **对话附件**（2026-09-10）：六入口复用 `conversation-attachments.js/css`，位于 composer 上方。原生折叠区、已有 tertiary 文字按钮、48 px 图片缩略图和明确传输确认；使用现有颜色/字体/间距 token。展开高度最多 40vh，文件列表独立滚动；失败保留已选附件，普通文字粘贴不受影响。附件确认不是资料保存，具体边界见 [多模态输入](MULTIMODAL_CONVERSATION_INPUTS.md)。
