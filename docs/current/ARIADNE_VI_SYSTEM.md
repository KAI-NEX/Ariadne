...
职位库的投递阶段（2026-09-11）：复用现有卡片网格，阶段按钮放在卡片正文链接之外；筛选与阶段按钮使用 caption、44 px 热区和胶囊圆角。进度对话框沿用 dialog/surface、字号与间距 token，保存/取消复用文字操作，不用颜色评价投递结果。实现见 `public/job-application.css`，行为见 [职位投递阶段](JOB_APPLICATION_STAGES.md)。
