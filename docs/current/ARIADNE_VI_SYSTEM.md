...
- **发送与回复**：整体对话页未勾选底部传输说明时，发送按钮可点击并触发浏览器原生校验浮提示、焦点移至复选框；勾选后不再重复询问同一接收方，Local/无资格/忙态仍禁用。通过发送前置检查后立即清空输入，等待期间的新草稿不被返回结果清除；两个整体对话页失败时恢复未被编辑的原草稿。共用消息组件只对本轮新回复快速逐字展开（按字素约 6 ms，总时长 120–1800 ms）；历史读取不重播，重绘续接进度，来源链接和原始回复保留。减少动态效果时直接完整显示。此为已校验回复的展示动效，模型处理期间仍显示等待状态，不代表 Provider 流式传输。
