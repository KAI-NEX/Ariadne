...
- **对话模型选择**（2026-09-10）：六入口复用 `conversation-model-selector.js/css`。文字区下增加 44 px 工具行，透明 plus 旁为 sliders 图标与模型/强度简称；textarea 原有最小高度与发送 42 px 保留，整体 field 随工具行增高。向上 popover 使用既有 surface/line/dialog shadow、字号、圆角与 44 px 控件热区，窄屏限宽并支持滚动；不新增图标资源或硬编码颜色。选项按已验证模型参数生成，Local/忙态/连接失败明确提示，Escape 恢复焦点。模型历史标签独立于正文，不修改消息内容。
