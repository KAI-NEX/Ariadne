...
- DeepSeek、Gemini、千问分别使用用户自己的 Key；六个领域执行复用现有能力 authority、校验、来源和人工保存契约。发送前仍需用户确认。Key 与请求原件只在请求内存/处理临时目录中使用，不写入持久存储；PDF 临时文件在处理结束后清理。结果在服务器内存短暂缓存，用于同次导入重试。
- 浏览器随机会话、Provider 与请求 Key 摘要共同隔离幂等、取消和删除状态；Key 变化也隔离。最多 32 个活跃命名空间，空闲 30 分钟后在后续请求时清理，正在执行的状态不清理。每个导入 registry 最多 128 个操作、1 MB 结果缓存，超预算保留回执、拒绝自动重复付费执行。服务重启清空临时状态，浏览器资料不受影响；不要自动重试模型 POST。
- Codex 留在用户自己的电脑。网页用配对码连接本机连接器，默认来源改为 `https://ariadne.kai-nex.com`；没有配对就不把 Codex 材料发送至网站服务。HTTPS 下的本地网络访问仍须在正式域名上线后实测。
- 2026-09-18 按用户要求开放 Gemini 3.7 Flash 和千问 Qwen 3.8 Max（百炼北京地域）。两者复用六个领域 adapter 的 schema、来源、授权和人工保存规则，使用各自固定官方 OpenAI 兼容端点；不自动切换模型。用户须先通过完整两页合成 PDF 读图和 JSON 连接验证。旧的单图试验接口/模块保留，不能当作业务质量证明。文本、图片、PDF、简单 DOCX 走原有领域边界；网页 Job 的复杂 Word 内嵌图像要求导出 PDF，避免悄悄丢图。PDF 在 Linux 使用 Poppler 完整逐页转图，最多 48 页、长边 2048 像素、40 MB 图像输出；超限拒绝整次处理，不截断。网页请求正文上限 41 MB（约 30 MB 原件及上下文）；本机既有上限不变。

## 新增 API 接入依据与范围

- Gemini 使用 [官方 3.7 Flash 模型契约](https://ai.google.dev/gemini-api/docs/models/gemini-3.7-flash)与 [OpenAI 兼容协议](https://ai.google.dev/gemini-api/docs/openai)，固定低推理强度，保留输出 token 预算；Google 的地区、账号和配额要求仍适用。
- 千问使用 [官方模型目录中的 Qwen 3.8 Max 图像理解能力](https://help.aliyun.com/zh/model-studio/models)与 [Chat Completions 协议](https://help.aliyun.com/zh/model-studio/qwen-api-via-openai-chat-completions)，显式关闭可选思考。本次使用官方仍支持的北京 `dashscope.aliyuncs.com`，其他地域 Key 尚未接入。
- 传输拒绝重定向，按 Provider/模型/请求凭据绑定；没有 Key 不使用其他服务的 Key，也不回退到服务器凭据。完整 PDF 转图复用既有路径；测试两页必须均返回准确文字，不按模型名称或列表判定通过。
- 六个领域已经通过真实领域校验到模拟 HTTP 回应的完整回归，图片、JSON 和工具返回均保留身份校验。真实账号的可用性在用户连接时检验，实际业务回答质量须用用户同意的材料另行验收。
