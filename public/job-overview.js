...
(async function jobOverviewPage() {
  ...
  function runtime() {
    ...
    el("job-overview-consent-copy").textContent = current.mode === "local" ? "当前为 Local。选择可用模型后，可以汇总与讨论职位。" : `允许将问题及当前 JD 内容发送至 ${current.provider} · ${current.model}；更新概况可能分批调用并产生 API 费用。不发送个人资料。`;
  ...
  function render() {
    ...
    el("job-overview-state").textContent = `当前 ${snapshot.records.length} 份 JD：${snapshot.confirmed_count} 份已保存，${snapshot.working_count} 份未确认草稿。${overview ? "概况已更新；结论为模型理解。" : "概况待更新；发送问题时也会更新。"}`;
    el("job-overview-summary").textContent = overview?.summary || (snapshot.records.length ? "先了解这些职位各自在做什么，再比较共同要求与差异。" : "还没有可汇总的 JD。添加真实职位描述后，可以在这里一起讨论。演示卡片不纳入概况。");
    ...
    el("job-overview-usage").textContent = last ? `最近一轮详细证据 ${last.context_coverage.included_jobs}/${last.context_coverage.total_jobs} 份，${last.context_coverage.truncated_jobs} 份节选；另参考当轮全量概况。上下文 ${Math.round(last.context_bytes / 1024)} KB，${last.calls} 次模型调用。` : "概况分批汇总全部当前 JD；对话按预算选取详细证据，保留原文和历史。";
    runtime();
  }
