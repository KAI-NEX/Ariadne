...
(async function () {
  ...
  try {
    ...
    const local = /^\/downloads\/Ariadne-Local-macOS-arm64-[0-9-]+\.zip$/.test(release.url);
    const github = /^https:\/\/github\.com\/KAI-NEX\/[A-Za-z0-9_.-]+\/releases\/download\/[A-Za-z0-9_.-]+\/Ariadne-Local-macOS-arm64-[0-9-]+\.zip$/.test(release.url);
    if ((!local && !github)
