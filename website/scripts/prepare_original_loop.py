...
for _ in range(args.end_frame):
 ...
 frames.append(frame)
cap.release();h,w=frames[0].shape[:2];n=len(frames);overlap=args.overlap
...
receipt={'source':str(args.source),'output':str(args.output),'fps':fps,'selected_source_frames':[0,n-1],'excluded_embedded_cut':n,'blend_frames':overlap,'output_frames':n-overlap,'duration':(n-overlap)/fps,'method':'smoothstep dissolve on original forward-moving frames; no geometric warp'}
